...
use cranelift_bforest::constant_hash::{probe, Table};
use std::ops::Range;
...
struct L1EntryTable<'a, OffT: Into<u32> + Copy>(&'a [Level1Entry<OffT>]);

impl<'a, OffT: Into<u32> + Copy> Table<Type> for L1EntryTable<'a, OffT> {
    fn len(&self) -> usize {
        self.0.len()
    }

    fn key(&self, idx: usize) -> Option<Type> {
        if self.0[idx].log2len != !0 {
            Some(self.0[idx].ty)
        } else {
            None
        }
...
struct L2EntryTable<'a, OffT: Into<u32> + Copy>(&'a [Level2Entry<OffT>]);

impl<'a, OffT: Into<u32> + Copy> Table<Opcode> for L2EntryTable<'a, OffT> {
    fn len(&self) -> usize {
        self.0.len()
    }
...
where
    ...
    OffT2: Into<u32> + Copy,
{
    let (offset, legalize) = match probe(
        &L1EntryTable(level1_table),
        ctrl_typevar,
        ctrl_typevar.index(),
    ) {
        Err(l1idx) => {
        ...
        Ok(l1idx) => {
            ...
            let offset = match level2_table.get(l1ent.range()) {
                Some(l2tab) => {
                    ...
                    match probe(&L2EntryTable(l2tab), opcode, opcode as usize) {
