...
use crate::cdsl::inst::{BoundInstruction, Instruction};
...
pub struct Var {
    ...
    /// The original 'typeof(x)' type variable that was created for this Var. This one doesn't
    /// change. The `type_var` above may be changed to another typevar by type inference.
    original_type_var: Option<TypeVar>,
}
...
impl Var {
    ...
    // XXX Could be put into the ctor directly, unless the Python code sometimes set Var.typevar by hand.
    pub fn get_or_create_typevar(&mut self) -> TypeVar {
        match &self.type_var {
            ...
            None => {
                ...
                self.original_type_var = Some(tv.clone());
                tv
            }
        }
    }
    ...
    pub fn has_free_typevar(&self) -> bool {
        // XXX Why simply not using tv.free_typevar() == tv?
        match &self.type_var {
            Some(tv) => tv.base.is_none() && Some(tv) == self.original_type_var.as_ref(),
            None => false,
        }
    }
...
impl Apply {
    ...
    fn inst_predicate(
        ...
        let mut pred = InstPredicate::new();
        for (format_field, &op_num) in iform.imm_fields.iter().zip(self.inst.imm_opnums.iter()) {
            ...
            pred = pred.and(InstPredicateNode::IsFieldEqual(
                format_field.member.into(),
        ...
        if self.value_types.len() > 1 {
            ...
            {
                pred = pred.and(make_typevar_check(&self.inst, type_var, bound_type));
            }
        }
    ...
    pub fn inst_predicate_with_ctrl_typevar(
        ...
        if !self.value_types.is_empty() {
            ...
            let type_check = if poly.use_typevar_operand {
                make_typevar_check(&self.inst, &poly.ctrl_typevar, bound_type)
            } else {
                InstPredicateNode::CtrlTypeVarCheck(bound_type.rust_name())
            };
...
pub enum InstPredicateNode {
    // Is field member (first member) equal to the actual argument (which name is the second
    // field)?
    IsFieldEqual(String, String),

    // Is the value argument (at index designated by the first member) the same type as the type
    // name (designated by the second argument)?
    TypeVarCheck(usize, String),

    // Is the controlling type variable the same type as the type name (designated by the string)?
    CtrlTypeVarCheck(String),

    // A combination of two other predicates.
    And(Vec<InstPredicateNode>),
}

fn make_typevar_check(
    inst: &Instruction,
    type_var: &TypeVar,
    ...
    let index = inst
        ...
        .unwrap()
        .0;
    InstPredicateNode::TypeVarCheck(index, value_type.rust_name())
}

impl InstPredicateNode {
    fn rust_predicate(&self) -> String {
        match self {
            InstPredicateNode::IsFieldEqual(field_name, arg) => {
                let new_args = vec![field_name.clone(), arg.clone()];
                format!("crate::predicates::is_equal({})", new_args.join(", "))
            }
            InstPredicateNode::TypeVarCheck(index, value_type_name) => format!(
                "func.dfg.value_type(args[{}]) == {}",
                index, value_type_name
            ),
            InstPredicateNode::CtrlTypeVarCheck(value_type_name) => {
                format!("func.dfg.ctrl_typevar(inst) == {}", value_type_name)
            }
            InstPredicateNode::And(nodes) => nodes
                .iter()
...
pub struct InstPredicate {
    node: Option<InstPredicateNode>,
}
...
impl InstPredicate {
    ...
    pub fn and(mut self, new_node: InstPredicateNode) -> Self {
        let node = self.node;
        ...
        self.node = Some(InstPredicateNode::And(and_nodes));
        self
    }
...
macro_rules! def {
    ...
    // inst.type(a, b, c)
    ($inst:ident.$type:ident($($src:ident),*)) => {
        ExprBuilder::apply(bind($inst, $type).into(), vec![$($src.clone().into()),*])
            .assign_to(Vec::new())
    };

    // (x, y) = inst(a, b, c)
    (($($dest:expr),*) = $inst:ident($($src:ident),*)) => {
        ExprBuilder::apply($inst.into(), vec![$($src.clone().into()),*])
            .assign_to(vec![$($dest.clone()),*])
    };

    // (x, y) = inst.type(a, b, c)
    (($($dest:expr),*) = $inst:ident.$type:ident($($src:ident),*)) => {
        ExprBuilder::apply(bind($inst, $type).into(), vec![$($src.clone().into()),*])
            .assign_to(vec![$($dest.clone()),*])
    };

    // x = inst(a, b, c)
    ($dest:ident = $inst:ident($($src:ident),*)) => {
        ExprBuilder::apply($inst.into(), vec![$($src.clone().into()),*])
            .assign_to(vec![$dest.clone()])
    };

    // x = inst.i32(a, b, c)
    ($dest:ident = $inst:ident.$type:ident($($src:ident),*)) => {
        ExprBuilder::apply(bind($inst, $type).into(), vec![$($src.clone().into()),*])
            .assign_to(vec![$dest.clone()])
    };
}
