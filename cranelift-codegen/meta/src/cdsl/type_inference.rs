    ...
    fn is_trivial(&self) -> bool {
        match self {
            ...
            Constraint::InTypeset(_, _) => {
                // XXX The way InTypeset are forged, they would always be trivial if we were
                // applying the same logic as the Python code did, so ignore this.
                self.is_concrete()
            }
        }
    }
...
impl TypeEnvironment {
    ...
    fn add_constraint(&mut self, constraint: Constraint) {
        ...
        match &constraint {
            Constraint::InTypeset(tv, _) => {
                assert!(tv.base.is_none());
                assert!(tv.name.starts_with("typeof_"));

                // InTypeset constraints can be expressed by constraining the typeset of a
                // variable. No need to add them to self.constraints.
                // XXX this was dead code in python or what?
                //self.get_equivalent(&tv).constrain_types_by_ts(ts.clone());
            }
            _ => {
    ...
    /// Record a that the free tv1 is part of the same equivalence class as tv2. The canonical
    /// representative of the merged class is tv2's canonical representative.
...
pub fn ti_xform(
    ...
    type_env = ti_def(def_pool.get(src), var_pool, type_env, &mut last_type_index)
    ...
    for (i, &def_index) in dst.iter().enumerate() {
        ...
        type_env = ti_def(def, var_pool, type_env, &mut last_type_index)
