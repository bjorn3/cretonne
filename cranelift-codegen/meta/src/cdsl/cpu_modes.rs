...
use crate::cdsl::xform::{XFormGroup, XFormGroupIndex, XFormGroups};
...
pub struct CpuMode {
    _name: &'static str,
    default_legalize: Option<XFormGroupIndex>,
    monomorphic_legalize: Option<XFormGroupIndex>,
    typed_legalize: HashMap<String, XFormGroupIndex>,
}
...
impl CpuMode {
    ...
    pub fn legalize_monomorphic(&mut self, group: &XFormGroup) {
        assert!(self.monomorphic_legalize.is_none());
        self.monomorphic_legalize = Some(group.id);
    }
    pub fn legalize_default(&mut self, group: &XFormGroup) {
        assert!(self.default_legalize.is_none());
        self.default_legalize = Some(group.id);
    }
    pub fn legalize_type(&mut self, lane_type: impl Into<LaneType>, group: &XFormGroup) {
        assert!(self
            .typed_legalize
