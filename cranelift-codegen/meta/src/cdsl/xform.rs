...
use crate::cdsl::type_inference::{ti_xform, TypeEnvironment};
...
///
/// A legalization pattern must have a source pattern containing only a single instruction.
pub struct Xform {
    pub src: DefIndex,
...
impl Xform {
    fn new(src: DummyDef, dst: Vec<DummyDef>) -> Self {
        ...
        let type_env = ti_xform(src, &dst, &def_pool, &mut var_pool).unwrap();
...
pub struct XFormGroup {
    pub name: &'static str,
    pub doc: &'static str,
    pub chain_with: Option<XFormGroupIndex>,
    pub isa_name: Option<&'static str>,
    pub id: XFormGroupIndex,
...
pub struct XFormGroupBuilder {
    name: &'static str,
    doc: &'static str,
    chain_with: Option<XFormGroupIndex>,
    isa_name: Option<&'static str>,
    pub custom_legalizes: HashMap<String, &'static str>,
    pub xforms: Vec<Xform>,
}

impl XFormGroupBuilder {
    ...
    pub fn chain_with(mut self, next_id: XFormGroupIndex) -> Self {
    ...
    pub fn legalize(&mut self, src: DummyDef, dst: Vec<DummyDef>) {
        let xform = Xform::new(src, dst);
        xform.verify_legalize();
        self.xforms.push(xform);
    }

    pub fn finish_and_add_to(self, owner: &mut XFormGroups) -> XFormGroupIndex {
        let next_id = owner.next_key();
        owner.add(XFormGroup {
            name: self.name,
            doc: self.doc,
...
pub struct XFormGroupIndex(u32);
entity_impl!(XFormGroupIndex);

pub struct XFormGroups {
    groups: PrimaryMap<XFormGroupIndex, XFormGroup>,
}
...
impl XFormGroups {
    ...
    pub fn add(&mut self, new_group: XFormGroup) -> XFormGroupIndex {
    ...
    pub fn get(&self, id: XFormGroupIndex) -> &XFormGroup {
        &self.groups[id]
    }
    pub fn get_mut(&mut self, id: XFormGroupIndex) -> &mut XFormGroup {
        self.groups.get_mut(id).unwrap()
    }
    fn next_key(&self) -> XFormGroupIndex {
        self.groups.next_key()
    }
    pub fn by_name(&self, name: &'static str) -> &XFormGroup {
        for group in self.groups.values() {
            if group.name == name {
                return group;
            }
...
fn test_double_custom_legalization() {
    ...
    let mut xform_group = XFormGroupBuilder::new("test", "doc");
    xform_group.custom_legalize(&dummy_inst, "custom 1");
    xform_group.custom_legalize(&dummy_inst, "custom 2");
}
