...
pub fn define(shared_defs: &mut SharedDefinitions) -> TargetIsa {
    ...
    let expand_flags = shared_defs.legalize_groups.by_name("expand_flags");
    let narrow = shared_defs.legalize_groups.by_name("narrow");
    let widen = shared_defs.legalize_groups.by_name("widen");
    let x86_expand = shared_defs.legalize_groups.by_name("x86_expand");
